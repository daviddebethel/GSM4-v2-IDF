...
- Foundation now includes first passing example suites: module tests under `firmware/components/gsm4_example/test`, executed via host test apps in `firmware/tests/host/unit` and `firmware/tests/host/mock`.
...
- DONE: 4.4 CI unit/mock execution uses real host suites (`unit-tests` and `mock-tests`) with profile build validation.
...
- DONE: foundation placeholders are fully replaced (`tools/test_unit.sh` and `tools/test_mock.sh` run real Unity/CMock host tests and emit JUnit + host logs).
